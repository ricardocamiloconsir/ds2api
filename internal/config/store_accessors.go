...
func (s *Store) RuntimeRateLimitPerIPRPM() int {
	s.mu.RLock()
	defer s.mu.RUnlock()
	if s.cfg.Runtime.RateLimitPerIPRPM > 0 {
		return s.cfg.Runtime.RateLimitPerIPRPM
	}
	if raw := strings.TrimSpace(os.Getenv("DS2API_RATE_LIMIT_PER_IP_RPM")); raw != "" {
		if n, err := strconv.Atoi(raw); err == nil && n > 0 {
			return n
		}
	}
...
func (s *Store) RuntimeRateLimitGlobalRPM() int {
	s.mu.RLock()
	defer s.mu.RUnlock()
	if s.cfg.Runtime.RateLimitGlobalRPM > 0 {
		return s.cfg.Runtime.RateLimitGlobalRPM
	}
	if raw := strings.TrimSpace(os.Getenv("DS2API_RATE_LIMIT_GLOBAL_RPM")); raw != "" {
		if n, err := strconv.Atoi(raw); err == nil && n > 0 {
			return n
		}
	}
...
func (s *Store) RuntimeRateLimitBurst() int {
	s.mu.RLock()
	defer s.mu.RUnlock()
	if s.cfg.Runtime.RateLimitBurst > 0 {
		return s.cfg.Runtime.RateLimitBurst
	}
	if raw := strings.TrimSpace(os.Getenv("DS2API_RATE_LIMIT_BURST")); raw != "" {
		if n, err := strconv.Atoi(raw); err == nil && n > 0 {
			return n
		}
	}
